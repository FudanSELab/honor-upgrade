...
type t =
  ...
  ; dependency_analyze: Nasus.DependencyAnalysisDomain.summary option Lazy.t }
[@@deriving fields]
...
let all_fields =
  ...
  Fields.to_list
    ...
    ~dependency_analyze:(fun f -> mk f DependencyAnalysis Nasus.DependencyAnalysisDomain.pp)
...
module SQLite = struct
  ...
  let make_eager =
    ...
    Fields.make_creator ~annot_map:data_of_sqlite_column ~biabduction:data_of_sqlite_column
      ...
      ~uninit:data_of_sqlite_column ~dependency_analyze:data_of_sqlite_column
  ...
  let lazy_load table ~proc_uid =
    ...
    ; dependency_analyze= lazy (load table ~proc_uid DependencyAnalysis) }
end
