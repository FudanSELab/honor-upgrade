...
let is_override x y =
  match x, y with
  | Block _, Block _ -> raise TODO
  | C _, C _ -> raise TODO
  | CSharp _, CSharp _ -> raise TODO
  | Erlang _, Erlang _ -> raise TODO
  | Hack _, Hack _ -> raise TODO
  | Java _, Java _ -> raise TODO
  | Linters_dummy_method, Linters_dummy_method -> raise TODO
  | ObjC_Cpp {method_name= method_name1; parameters= _parameters1}, ObjC_Cpp {method_name= method_name2; parameters= _parameters2} ->
      (String.(=) method_name1 method_name2) && (true)
  | Python _, Python _ -> raise TODO
  | WithFunctionParameters _, WithFunctionParameters _ -> raise TODO
  | _ -> false
...
let to_json (procname : t) : string =
  match procname with
  | C {name} -> (
    match IR__QualifiedCppName.to_list name with
    | [] ->
        raise (Failure "does not have function name")
    | funcname :: _ ->
        funcname )
  | ObjC_Cpp {method_name} ->
      method_name
  | _ ->
      raise (Failure "unhandle condition")