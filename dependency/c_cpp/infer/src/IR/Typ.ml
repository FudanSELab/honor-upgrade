...
let to_json (typ : t) : ATDGenerated.Dependency_t.type_name =
  let rec typ_to_kind typ =
    let open ATDGenerated.Dependency_t in
    match typ.desc with
    | Tstruct (CStruct _) ->
        Struct
    | Tstruct (CUnion _) ->
    ...
    | Tfloat _ -> Enum
    | Tvoid -> Enum
    | Tarray {elt} -> typ_to_kind elt
    | TVar _ -> Enum
    | Tstruct _ -> raise (Failure "only support C/C++ for now")
    | Tfun -> Enum
    (* FIXME: temp solution *)
  in
  let name = to_string typ in
  let kind = typ_to_kind typ in
  {name; kind}
