...
let to_json (pdesc : t) : ATDGenerated.Dependency_t.func =
  let attri = get_attributes pdesc in
  let procname = Procname.to_json attri.proc_name in
  let func_type : ATDGenerated.Dependency_t.func_type =
    let args_type = List.map attri.formals ~f:(fun (_, typ, _) -> Typ.to_string typ) in
    let return_type = Typ.to_string attri.ret_type in
    let class_name =
      match attri.proc_name with
      | C {name} -> (
  ...
  in
  let loc = Location.to_json attri.loc in
  { func_name= procname
  ; func_type= func_type
  ; location= loc }
