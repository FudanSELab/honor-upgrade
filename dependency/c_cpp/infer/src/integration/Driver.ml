...
let serialize_dependency () : unit =
  let procnames_to_pdescs procnames =
    List.rev_map procnames ~f:(fun procname -> procname |> Procdesc.load |> Option.value_exn)
  in
  let all_files = SourceFiles.get_all ~filter:(fun _ -> true) () in
  let all_pnames = List.fold all_files ~init:[] ~f:(fun acc file ->
    acc @ SourceFiles.proc_names_of_source file) in
  let all_pdescs = List.fold all_pnames ~init:[] ~f:(fun acc pname ->
    acc @ procnames_to_pdescs [pname]
  ) in
  (* NOTE: that you can not get all tenv in C++ *)
  let _ = Tenv.merge in
  let get_tenv file =
    file |> Exe_env.get_source_tenv (Exe_env.mk ()) |> Option.value_exn in
  let _all_tenvs =
    List.fold all_files ~init:(Tenv.create ()) ~f:(fun acc file ->
      Tenv.merge ~src:(get_tenv file) ~dst:acc; acc)
  in

  (* TODO: to_json summary *)
  print_endline "going to print summary" ;
  (* get all the summary *)
  (* FIXME: "stack overflow" *)
  let sums = ref [] in
    Summary.OnDisk.iter_specs ~f:(fun sum -> sums := sum :: !sums) ;
  let to_call acc (sum : Summary.t) = 
    match Lazy.force sum.payloads.dependency_analyze with
    | Some dep_sum ->
        (* FIXME: "stack overflow" -- HERE --*)
        Nasus.DependencyAnalysisDomain.to_json dep_sum :: acc
    | None ->
        acc
  in
  let calling =
    List.fold !sums ~init:[] ~f:to_call
  in
  let str =
    {calling; inheritance= []; reference= []} |> 
    ATDGenerated.Dependency_j.string_of_dependency
  in
  let file = Stdlib.open_out "calling.json" in
  Stdlib.output_string file str ; Stdlib.close_out file ;

  (* TODO: Reference Analysis *)
  print_endline "going to print reference analsis" ;
  let refs = Nasus.ReferingAnalysis.solve all_pdescs in
  let json = Nasus.ReferingAnalysis.to_json refs in
  let str =
    ATDGenerated.Dependency_j.string_of_dependency {calling= []; inheritance= []; reference= json} in
  let file = Stdlib.open_out "reference.json" in
    Stdlib.output_string file str ; Stdlib.close_out file ;

  (* TODO: Points-to analysis *)
  print_endline "going to print pointsTo"
  (* let files = SourceFiles.get_all ~filter:(fun _ -> true) () in
  List.iter files ~f:(fun file ->
      match Exe_env.get_source_tenv (Exe_env.mk ()) file with
      | Some _ ->
          let procs = SourceFiles.proc_names_of_source file in
          let _ = List.map procs ~f:(fun proc ->
            let func = match Procdesc.load proc with | Some f -> f | None -> assert false in
            Nasus.PointsTo.solve func) in
          ()
      | None ->
          assert false ); *)

  (* TODO: Devirtualizer *)
  (* print_endline "going to print devirtualizer";
     let files = SourceFiles.get_all ~filter:(fun _ -> true) () in
     List.iter files ~f:(fun file ->
         match Exe_env.get_source_tenv (Exe_env.mk ()) file with
         | Some tenv ->
             ...
             let _ = List.map procs ~f:(fun proc ->
               let func = match Procdesc.load proc with | Some f -> f | None -> assert false in
               Devirtualizer.process func tenv) in
             ()
         | None ->
             assert false ); *)
  (* let _ = Dependencies.Subtyping.find_subtypes in *)

  (* TODO: to_json tenv *)
  (* print_endline "going to print tenv" ;
  let files = SourceFiles.get_all ~filter:(fun _ -> true) () in
  List.iter files ~f:(fun file ->
      match Exe_env.get_source_tenv (Exe_env.mk ()) file with
      | Some tenv ->
          Tenv.pp F.std_formatter tenv
      | None ->
          assert false ) ; *)

  (* TODO: print all tenvs *)
  (* print_endline "going to print all tenvs" ;
  Tenv.pp F.std_formatter all_tenvs *)
...
let analyze_and_report ~changed_files mode =
  ...
  | (Analyze | Run), _ ->
      if SourceFiles.is_empty () then error_nothing_to_analyze mode
        ...
        if Config.test_dependency then serialize_dependency () ;
