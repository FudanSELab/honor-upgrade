...
import edu.fdu.se.core.miningchangeentity.statement.ReturnChangeEntity;
import edu.fdu.se.lang.common.ClassLookupTbl;
import edu.fdu.se.lang.common.MethodLookupTbl;
import org.eclipse.jdt.core.dom.ASTNode;

@ClassLookupTbl(key = ChangeEntityDesc.StageIIENTITY.ENTITY_RETURN_STMT)
public class MatchReturn extends MatchStatement {

    public ReturnChangeEntity newEntity() {
        return new ReturnChangeEntity(mBean);
    }


    @MethodLookupTbl(key = ASTNode.RETURN_STATEMENT)
    public void matchReturnTopDown(MiningActionData fp, Action a) {
        init();
        traverseTopDown(fp, a, TraverseWays.TopDown.TYPE_I, 0);
        newBean(a);
        ReturnChangeEntity code = newEntity();
        setTopDown(code, a, fp, 0);
    }


    @MethodLookupTbl(key = ASTNode.RETURN_STATEMENT)
    public void matchReturnBottomUpNew(MiningActionData fp, Action a, Tree queryFather, int treeType, Tree traverseFather) {
    ...
    @MethodLookupTbl(key = ASTNode.RETURN_STATEMENT)
    public void matchReturnBottomUpCurr(MiningActionData fp, Action a, ChangeEntity changeEntity, Tree traverseFather) {
